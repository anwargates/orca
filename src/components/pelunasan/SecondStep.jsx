...
export const SecondStep = ({ form, selected, handler }) => {
  ...
  const currentPaymentMethod = dataPayment.find((item) => {
    console.log(item)
    console.log(form.metodePelunasan)
    return item.value.includes(form.values.metodePelunasan)
  })
  ...
  const handleCreateDoc = async (url) => {
    ...
    await setDoc(doc(db, 'payments', orderId), {
      ...
      statusCode: 5,
      buktiPelunasan: url,
      status: 'Verifikasi Pelunasan Diterima',
  ...
  const handleNotify = () => {
    ...
    set(newNotificationRef, {
      title: 'Pembayaran Pelunasan baru!',
  ...
  return (
    <div className='grid grid-cols-3 grid-rows-1 gap-10 md:p-10 lg:pl-20 xl:pl-32 lg:pr-20 h-[800px] relative'>
      <LoadingOverlay
        visible={pending}
        loader={
          <Loader
        ...
        <div className='w-full h-full relative'>
          ...
          <div className='relative w-full h-full bg-transparent flex flex-col items-center justify-around'>
            ...
            <div className='w-[320px] h-[123px] bg-white shadow-md rounded-lg text-left flex flex-col justify-center items-start px-7 gap-2 relative'>
              ...
              <p className='font-bold text-4xl'>{form.values.pelunasan?.toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.')}</p>
